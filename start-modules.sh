...
cd "$CONTAINER_ROOT" || {
  echo -e "${RED}[Orchestrator] Cannot cd to ${CONTAINER_ROOT}${NC}"
  exit 1
}
...
AUTOUPDATE_STATUS="${AUTOUPDATE_STATUS:-false}"
...
# 3) Bundled MariaDB then MediaWiki auto-install (same container)
DB_STATUS="${DB_STATUS:-true}"
if is_enabled "$DB_STATUS" && [[ -x "modules/database/start.sh" ]]; then
  header "Running module: database"
  if ! modules/database/start.sh; then
    echo -e "${RED}[Orchestrator] Database module failed (see [Database] messages above).${NC}"
    exit 1
  fi
fi

# MediaWiki: always sync site URL; auto-install only when AUTOINSTALL_STATUS is enabled.
if [[ -x "modules/mediawiki/start.sh" ]]; then
  header "Running module: mediawiki"
  modules/mediawiki/start.sh
fi

# 4) Execute other modules (except autoupdate, logcleaner, database, mediawiki, nginx)
for module_dir in modules/*/; do
  module_name=$(basename "$module_dir")
  [[ "$module_name" == "autoupdate" || "$module_name" == "logcleaner" || "$module_name" == "database" || "$module_name" == "mediawiki" || "$module_name" == "nginx" || "$module_name" == "git" || "$module_name" == "lib" ]] && continue
...
# 5) Run nginx module last (blocking)
