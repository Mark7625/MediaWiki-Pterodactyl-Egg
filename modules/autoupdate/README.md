...
This module checks for and applies updates from the MediaWiki Pterodactyl egg GitHub repository (default: [Mark7625/MediaWiki-Pterodactyl-Egg](https://github.com/Mark7625/MediaWiki-Pterodactyl-Egg)). It compares the short commit SHA in `VERSION` with the latest commit on `EGG_BRANCH` and clones/pulls changed files on startup when enabled.
...
| `EGG_REPO` | `https://github.com/Mark7625/MediaWiki-Pterodactyl-Egg` | GitHub repository URL |
| `EGG_BRANCH` | `master` | Branch to track for updates |
...
2. **GitHub / git**: Resolves latest commit SHA via `git ls-remote` or GitHub API (`…/commits/master` by default)
3. **Comparison**: Compares current vs latest version
4. **Git clone / archive**: If update available, clones the repo (or downloads branch tarball) and copies allowed paths
...
### Version API
```bash
GET https://api.github.com/repos/Mark7625/MediaWiki-Pterodactyl-Egg/commits/master
```

### Diff API
```bash
git clone --depth 1 -b master https://github.com/Mark7625/MediaWiki-Pterodactyl-Egg
```
